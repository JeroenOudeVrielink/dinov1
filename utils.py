...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:
            ...
            # If using the conv head and the resolution is 3x3, we need to interpolate to 7x7
            if _out.size()[-2:] == (3, 3):
                _out = nn.functional.interpolate(
                    _out, size=(7, 7), mode="bilinear", align_corners=False
                )
            # accumulate outputs
...
class MultiCropWrapperConvTransposeHead(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    def forward(self, x):
        ...
        start_idx, output = 0, torch.empty(0).to(x[0].device)
        for end_idx in idx_crops:
            x = torch.cat(x[start_idx:end_idx])
            if x.size()[-2:] == (3, 3):
                x = nn.functional.interpolate(
                    x, size=(7, 7), mode="bilinear", align_corners=False
                )
